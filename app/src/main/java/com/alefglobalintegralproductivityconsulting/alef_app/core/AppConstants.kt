package com.alefglobalintegralproductivityconsulting.alef_app.core

class AppConstants {
    companion object {
        const val IS_CLOSED_PREVIEW_TIME: String = "IS_CLOSED_PREVIEW_TIME"
        const val IS_LOGIN_USER: String = "IS_LOGIN_USER"
        const val USER_TOKEN: String = "USER_TOKEN"
        const val DETAILS_VACANT: String = "DETAILS_VACANT"

        const val VACANT_INFO_EXTRA: String = "VACANT_INFO_EXTRA"

        const val SEARCH_GENERAL: String = "SEARCH_GENERAL"

        const val SEARCH_LOCATION: String = "SEARCH_LOCATION"

        // Credentials Google
        const val ID_GOOGLE_CLIENT = "39455255105-agu55okmn8ieovl8jtpc3hv1ljluoics.apps.googleusercontent.com"
        const val CLIENT_SECRET = "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"
        const val USER_ID_GOOGLE: String = "USER_ID_GOOGLE"
        const val USER_PICTURE_PROFILE: String ="USER_PICTURE_PROFILE"
    }
}
